...
>>>>>>> 30fe2947159e5008292470a639999787c11ea345
```
...
In addition to linear assumption check, T-test and F- test can be used to further check and improve our model. First of all, T-test is used to determine whether individual predictors in the model are statistically significant in explaining dependent variables . To begin with, we choose our null hypothesis -  $H_0:\beta=0$, $H_1:\beta\neq0$ and $\alpha$ being 0.05
, then to complete T-test, we need to examine each predictor individually, after conducting t-test on each predictor , we get p-value for each predictor and if p-value is smaller then our alpha, reject $H_0$ which means we keep that predict, if the predictor we are examining is 
categorical variable, then drop all terms for these categorical variables and drop this categorical variable, else drop this term and the corresponding variable if only this term contains this variable. After the T test, the F test evaluates whether the independent variables, as a group, explain a significant portion of the variation in the dependent variable.
